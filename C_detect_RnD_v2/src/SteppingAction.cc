...
{
  if (!fSiPMScoringVolume || !fPMTScoringVolume || !fOpticalVolume) {
    ...
    fOpticalVolume = detConstruction->GetOpticalFiberVolume();
  }
  ...
  if (volume != fSiPMScoringVolume && volume != fPMTScoringVolume && volume !=fOpticalVolume ){
    return;
    } 
